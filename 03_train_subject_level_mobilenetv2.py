"""
03_train_subject_level_mobilenetv2.py
--------------------------------------
Loads the preprocessed LVOT/RVOT beat images produced by
01_preprocess_beats.py, and trains/evaluates the MobileNetV2-based
transfer-learning model with a SUBJECT-LEVEL (patient-level) split.
...
def load_dataset():
    ...
    for _, row in manifest.iterrows():
        ...
        # Load as grayscale (source images are grayscale), then replicate
        # to 3 channels since MobileNetV2 requires RGB-shaped input
        # (matches the original notebook's np.repeat(..., 3, axis=-1) step).
...
#    frozen MobileNetV2 backbone + custom classification head)
# ------------------------------------------------------------------
def create_model():
    base_model = MobileNetV2(weights='imagenet', include_top=False, input_shape=(128, 128, 3))
    base_model.trainable = False  # frozen backbone, same as original notebook
...
def main():
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(PROCESSED_DIR / "training_curves_mobilenetv2.png", dpi=150)
    plt.show()
    ...
    plt.savefig(PROCESSED_DIR / "roc_curve_mobilenetv2.png", dpi=150)
    plt.show()
    ...
    plt.savefig(PROCESSED_DIR / "confusion_matrix_mobilenetv2.png", dpi=150)
    plt.show()
